...
def get_mongodb_connection():
    ...
    for attempt in range(max_retries):
        try:
            ...
            items_collection = db["listed_items"]
            print(f"✅ Connected to MongoDB: {DATABASE_NAME}.listed_items")
...
def store_listed_item(data):
    """Store listed item in MongoDB and log to console"""
    ...
    if isinstance(data, dict):
        ...
        document.update({
            ...
            "tradehold": data.get('tradehold', 0)
        })
    
    if items_collection is not None:
        try:
            result = items_collection.insert_one(document)
            print(f"✅ Stored listed item in MongoDB with ID: {result.inserted_id}")
        except Exception as e:
            print(f"❌ Failed to store in MongoDB: {e}")
    
    log_listed_item(data)

def log_listed_item(data):
    """Log listed item with timestamp"""
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(f"\n[{timestamp}] 📈 NEW LISTING:")
    ...
    if isinstance(data, dict):
        ...
        if suggested_price:
            print(f"  💡 Suggested: ${suggested_price/1000:.3f}")
        print(f"  🏷️  Wear: {wear}")
    ...
    async with websockets.connect(BITSKINS_WS_URL) as ws:
        ...
        while True:
            ...
            if not authenticated and action and action.startswith("WS_AUTH"):
                authenticated = True
                await ws.send(json.dumps(["WS_SUB", "listed"]))
                print("📡 Subscribed to 'listed' events only")

            elif authenticated and action == "listed":
                store_listed_item(data)

if __name__ == "__main__":
    print("🚀 Starting BitSkins LISTED Items Bot...")
    asyncio.run(bitskins_listed_bot())
